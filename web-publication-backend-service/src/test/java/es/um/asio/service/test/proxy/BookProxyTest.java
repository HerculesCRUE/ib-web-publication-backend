...
public class BookProxyTest {
	...
	static class BookProxyTestConfiguration {
		...
		@Bean
		@Qualifier("delegate")
		public BookMapperDecorator bookMapper() {
	...
	public void setUp() {
		...
		Mockito.when(this.service.findPaginated(filter, pageable)).thenAnswer(invocation -> {
			...
			bindings.put("edition", this.propertyMap("literal", "es", ""));
			bindings.put("endPage", this.propertyMap("literal", "es", ""));
			bindings.put("iccn", this.propertyMap("literal", "es", ""));
			bindings.put("placeOfPublication", this.propertyMap("literal", "es", ""));
	...
	public void proxyTest() {
		Page<BookDto> page = this.mapper.convertPageFusekiResponseToDto(this.service.findPaginated(filter, pageable));
